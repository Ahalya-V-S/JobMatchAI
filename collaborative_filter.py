...
class CollaborativeFilter:
    """Collaborative filtering using matrix factorization (SVD) adapted to your dataset"""
    ...
    def _generate_interactions(self, data):
        """Generate synthetic user-item interactions based on available columns"""
        np.random.seed(42)
        ...
        for user_id in range(n_users):
            ...
            # Score all jobs based on preference
    ...
    def recommend(self, user_profile, n_recommendations=10):
        """Simple CF recommendation for a synthetic user profile"""
        if self.item_factors is None:
            return None

        # Create synthetic user vector
        user_scores = np.ones(self.user_item_matrix.shape[1])
        if 'job_level' in self.data.columns and 'job_level' in user_profile:
            user_scores += (self.data['job_level'] == user_profile['job_level']).astype(float)*2
        if 'job_type' in self.data.columns and 'job_type' in user_profile:
            user_scores += (self.data['job_type'] == user_profile['job_type']).astype(float)*1.5

        # Weighted sum over item factors
        synthetic_user = np.dot(user_scores, self.item_factors) / np.sum(user_scores)

        # Compute similarity with all users
        user_similarities = cosine_similarity([synthetic_user], self.user_factors)[0]
        top_user_idx = np.argsort(user_similarities)[::-1][:5]  # top 5 similar users

        # Aggregate top items from similar users
        job_scores = {}
        for uidx in top_user_idx:
            ratings = self.user_item_matrix[uidx].toarray()[0]
            for jdx, r in enumerate(ratings):
                if r >= 4.0:
                    job_scores[jdx] = job_scores.get(jdx, 0) + r * user_similarities[uidx]

        # Return top N jobs
        top_jobs = sorted(job_scores.items(), key=lambda x: x[1], reverse=True)[:n_recommendations]
